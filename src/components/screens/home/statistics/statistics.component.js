...
export class Statistics extends ChildComponent {
  constructor() {
    ...
    this.element = renderService.htmlToElement(template, [new Heading('Statistics')], styles);
  ...
  fetchData() {
    this.statisticService.main((data) => {
      ...
      statisticsItemsElement
        .append(new StatisticsItem('Income:', formatToCurrency(data[0].value), 'green').render())
        .append(new StatisticsItem('Expense:', formatToCurrency(data[1].value), 'purple').render());
    });
  }
